...
class FileImportSeeder extends Seeder
{
    ...
    protected function readWordClassGroup(\SimpleXMLElement $groupXML): void
    {
        ...
        if (isset($groupXML->layout->table)) {
            ...
            $tablePosition = 0;
            $columnPosition = 0;
            foreach (iterator_to_array($groupXML->layout->table, false) as $i => $tableXML) {
    ...
    /**
     * Parse the relevant XML data in the inflections config into an InflectionTable model
     */
    protected function readInflectionTable(
        ...
        int &$tablePosition,
        int &$columnPosition,
        ?InflectionTable &$previousModel,
        ?\SimpleXMLElement &$previousXML,
    ): void
    {
        /**
        ...
        $previousModel = $tableModel;
        $previousXML = $tableXML;
    }
